// Grader.c Created by: Joe Ermatinger 11/06/2018   
/* Input: Keyboard
   Output: Screen
   ...
   Input will come from the keyboard using scanf(). Output will be displayed on 
   the screen using printf(). The user will be prompted to answer each question 
...
int main(){
	...
	/* Display Heading */
	printf("Quiz Application by Joe Ermatinger CIS185\n\n");
	printf("Enter your answers below. Choices are A, B, C, D, or E. Use uppercase!\n");
	...
	for(; i < 10; i++){
		...
		/* Incorrect Output Section */
			...
			if(incorrect == 3){
				printf("\a\tTake your time & be more careful!\n"); 
			}
			else if(incorrect == 5){
				printf("\a\tYou call that trying? You can do better than that!\n"); 
			}
			else if(incorrect == 7){
				printf("\a\tIs that really the best you can do? Try harder!\n");
			}
			else if(incorrect == 9){
				printf("\a\tAt least you didn't get them all wrong, yet. Try again!\n");		
			}		
		}	
		printf("\n");
	}
	...
	/* Display Final Grade */
	printf("\nStudent got %d out of 10 correct\n\n", ncorrect);                  /* Display Number Correct */
	printf("The percentage correct is %.2f\n\n", percentage);                    /* Display Percentage */
	printf("The letter grade is %c", getGrade(percentage));                      /* Display Grade */
}
...
char answer;	
	do {
			printf("What is your answer to question # %d? ", questionNum);                          /* Prompt and Input Answer */
			scanf(" %c", &answer);
			if (!validAnswer(answer))
			{
				printf("\tAnswer invalid! Choices are A, B, C, D, or E. Use uppercase!\n");         /* Display Answer Invalid */
			}		
